...
V_full_mig_func <- function(
  ...
  for (j in 1:nrow(mig_dec_mat)) {
    ...
    cons1_fb_mig <- array(NA, dim = c(vil_S, mig_S, (g + 1)))
    cons2_fb_mig <- array(NA, dim = c(vil_S, mig_S, (g + 1)))
    for (l in 1:(g + 1)) {
      cons1_fb_mig[,,l] <- total_inc / (1 + q[l]^(- 1 / sig))
      cons2_fb_mig[,,l] <- total_inc - cons1_fb_mig[,,l]
    }
    
    for (k in 1:mig_S) {
      total_util_mat[,,j] <- (util(cons1_fb_mig[,k,], sig) + util(cons2_fb_mig[,k,], sig)) * mig_R[k]
      util1_mat[,,j] <- util(cons1_fb_mig[,k,], sig) * mig_R[k]
      util2_mat[,,j] <- util(cons2_fb_mig[,k,], sig) * mig_R[k]
    }
...
for (j in 1:nrow(mig_dec_mat)) {
  # total resources, depending on s, q, and j
  total_inc <- mig_dec_mat[j, 1] * (
    matrix(vil_inc_mat[,1], nrow = vil_S, ncol = mig_S) +
        matrix(mig_inc_mat[,1], nrow = vil_S, ncol = mig_S, byrow = TRUE)
  ) +
