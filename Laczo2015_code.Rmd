---
title: "Dynamic limited commitment model in Laczó (2015)"
...
@Ligon2015 argues that, as in @Ligon2002, the evolution of relative Pareto weights is fully characterized by state-dependent intervals, which give the weights in the case where PCs are binding.
...
This property is called "amnesia" (@Kocherlakota1996).
...
$$
  \frac{u_2'(c_{2}^h(s))}{u_1'(c_{1}(s))} = \overline{x}^h(s).
$$
...
Here I test if the function above works or not:
...
# Output: values under risk sharing (full and limited commitment) and intervals
V_output <- V_lc_all_func(inc1, P1, inc2, P2, g, delta, sigma1, sigma2, pcphi, util, util_prime)
...
Also, the penalty under autarky is absent in the original numerical exercise.
...
start_time <- Sys.time()
delta_seq <- seq(0.8, 0.999, by = 0.001)
...
V_static_lc_func <- function(
  ...
  # Value function iterations ================
  # Initial guess is expected utilities under full risk sharing
  V1 <- cbind(Uaut[, 1], V1_full[qmax], V1_full[, x0_close])
  V2 <- cbind(Uaut[, 2], V2_full[qmax], V2_full[, x0_close])
  V1_new <- matrix(nrow = S, ncol = 3)
  V2_new <- matrix(nrow = S, ncol = 3)
  ...
  while ((diff > 1e-10) && (iter <= maxiter)) {
    ...
    # Values at the lower bounds are values of autarky
    V1_new[, 1] <- Uaut[, 1]
    V2_new[, 1] <- Uaut[, 2]
    ...
    for (k in 1:S) {
      ...
      calc_diff_val_aut_1 <- function(x) {
        ...
        # difference between 
        diff <- util(cons_x, sigma1) + delta * R[k,] %*% V1[, 1] - Uaut[k, 1]
        return(diff)
      }
      ...
      calc_diff_val_aut_2 <- function(x) {
        ...
        # difference between 
        diff <- util(inc_ag[k] - cons_x, sigma2) + delta * R[k,] %*% V2[, 1] - Uaut[k, 2]
        return(diff)
      }   
      ...
      # Derive the relative Pareto weight that satisfies the equation (1)
      # in the supplementary material of Laczo (2015)
      print(c(calc_diff_val_aut_1(qmax), calc_diff_val_aut_1(qmin)))
      if ((calc_diff_val_aut_1(qmax) > 0) && (calc_diff_val_aut_1(qmin) < 0)) {
        x_low <- uniroot(calc_diff_val_aut_1, c(qmin, qmax), tol = 1e-8, maxiter = 100)$root
        x_int[k, 1] <- x_low
      } else if (calc_diff_val_aut_1(qmax) <= 0){
      ...
      V2_new[k, 2] <- util((inc_ag[k] - cons_x_low), sigma2) + delta * R[k,] %*% V2[, 2]
      ...
      V1_new[k, 2] <- util(cons_x_high, sigma1) + delta * R[k,] %*% V1[, 2]
      
      if (x0 <= x_low) {
        V1_new[k, 3] <- Uaut[k, 1]
        V2_new[k, 3] <- util((inc_ag[k] - cons_x_low), sigma2) + delta * R[k,] %*% V2[, 2]
      } else if (x0 >= x_high) {
        V1_new[k, 3] <- util(cons_x_high, sigma1) + delta * R[k,] %*% V1[, 2]
        V2_new[k, 3] <- Uaut[k, 2]
      } else {
        V1_new[k, 3] <- util(cons1_x0[k], sigma1) + delta * R[k,] %*% V1[, 3]
        V2_new[k, 3] <- util((inc_ag[k] - cons1_x0[k]), sigma2) + delta * R[k,] %*% V2[, 3]
      }
...
```{r, include = FALSE, eval = FALSE}
V_static_output <- V_static_lc_all_func(
  inc1, P1, inc2, P2, g,
  0.95, sigma1, sigma2, pcphi,
  util, util_prime, x0
  )

log(V_static_output[[5]])
```
...
# Function that returns intervals and value functions
...
lc_func <- function(
  ...
  while ((diff > 1e-10) && (iter < maxiter)) {
    ...
    for (k in 1:S) {
      ...
      if (q_ind_low <= (g + 1)) {
        if ((calc_diff_val_aut_2(qmin) > 0) && calc_diff_val_aut_2(qmax) < 0) {
          ...
          # Index of the point on the grid of relative Pareto weights (q) closest to x_high
          q_ind_high <- which.min(abs(x_high - q))
          if (x_high > q[q_ind_high]) {
            q_ind_high <- q_ind_high + 1
          }
      ...
      if ((q_ind_low + 1) < (q_ind_high - 1)) {
        if (dynamic == TRUE) {
          ...
          x0_close <- which.min(abs(x0 - q))
          V1_new[k, (q_ind_low + 1):(q_ind_high - 1)] <- 
            util(cons1[k, x0_close], sigma1) + 
            delta * R[k,] %*% V1[, x0_close]
          V2_new[k, (q_ind_low + 1):(q_ind_high - 1)] <- 
            util(inc_ag[k] - cons1[k, x0_close], sigma2) + 
            delta * R[k,] %*% V2[, x0_close]
        }
      }
    ...
    if (iter == maxiter) {
      print("Reach the maximum limit of iterations!")
    }
  ...
  return(list(V1_lc, V2_lc, V1_full, V2_full, x_int))
}

start_time <- Sys.time()
aa <- lc_func(inc1, P1, inc2, P2, g, q, 0.8, sigma1, sigma2, pcphi)
end_time <- Sys.time()
print(end_time - start_time)
log(aa[[5]])
...
output <- vector(mode = "list", length = length(delta_seq))
for (i in seq_along(delta_seq)) {
  output[[i]] <- lc_func(inc1, P1, inc2, P2, g, q, delta_seq[i], sigma1, sigma2, pcphi)
}
end_time <- Sys.time()
print(end_time - start_time)
...
ggplot() +
  geom_line(aes(x = delta_seq, y = log(x_int_array[2,1,]))) +
  geom_line(aes(x = delta_seq, y = log(x_int_array[2,2,])))

ggplot() +
  geom_line(aes(x = delta_seq, y = log(x_int_array[1,1,]))) +
  geom_line(aes(x = delta_seq, y = log(x_int_array[1,2,]))) +
  geom_line(aes(x = delta_seq, y = log(x_int_array[2,1,]))) +
  geom_line(aes(x = delta_seq, y = log(x_int_array[2,2,]))) +
  geom_line(aes(x = delta_seq, y = log(x_int_array[3,1,]))) +
  geom_line(aes(x = delta_seq, y = log(x_int_array[3,2,]))) +
  geom_line(aes(x = delta_seq, y = log(x_int_array[4,1,]))) +
  geom_line(aes(x = delta_seq, y = log(x_int_array[4,2,]))) 
