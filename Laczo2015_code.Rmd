---
title: "Computation of limited commitment model, based on Laczó (2015)"
...
## Bonus 3: Three-state income process

```{r}

# income shocks and their transition probabilities of the household
inc1 <- c(1/4, 2/2, 5/4)
P1 <- matrix(rep(c(0.2, 0.6, 0.2), 3), nrow = 3, byrow = TRUE)

# income shocks and their transition probabilities of the village
inc1 <- c(1/4, 2/2, 5/4)
P2 <- matrix(rep(c(0.2, 0.6, 0.2), 3), nrow = 3, byrow = TRUE)

delta <- 0.85 # time discount factor
sigma1 <- 1.0 # coefficient of relative risk aversion of HH1
sigma2 <- 1.0 # coefficient of relative risk aversion of HH2
pcphi <- 0.0 # punishment under autarky
...
output_3_states <- V_lc_all_func(
   inc1, P1, inc2, P2, g, 
   delta, sigma1, sigma2, pcphi, util, util_prime
)

saveRDS(output_3_states, file.path("RDSfiles/laczo2015_dynamic_lc_3_states_output.rds"))
```

```{r, include = FALSE}
output_3_states <- readRDS("RDSfiles/laczo2015_dynamic_lc_3_states_output.rds")
```

```{r}
x_int_3_states <- output_3_states[[5]]
rownames(x_int_3_states) <- c(
  "Low, Low", "Meidum, Low", "High, Low",
  "Low, Medium", "Meidum, Medium", "High, Medium",
  "Low, High", "Meidum, High", "High, High"
)
colnames(x_int_3_states) <- c("x_low", "x_high")
kable(x_int_3_states, digits = 3)
```
